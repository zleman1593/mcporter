...
describe("config imports", () => {
	it("merges external configs with first-wins precedence", async () => {
		...
		expect(
			shared?.command.kind === "http"
				? shared.command.url.toString()
				: undefined,
		).toBe("https://cursor.local/mcp");

		const cursorOnly = servers.find((server) => server.name === "cursor-only");
		expect(cursorOnly?.command.kind).toBe("http");
		expect(
			cursorOnly?.command.kind === "http"
				? cursorOnly.command.url.toString()
				: undefined,
		).toBe("https://local.override/cursor");

		const codexOnly = servers.find((server) => server.name === "codex-only");
		expect(codexOnly?.command.kind).toBe("stdio");
		expect(
			codexOnly?.command.kind === "stdio"
				? codexOnly.command.command
				: undefined,
		).toBe("codex-cli");
	});
});
